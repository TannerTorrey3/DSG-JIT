# experiments/exp35_auto_denoise.py
"""
Self-tuning SE(3) measurement denoiser.

Estimates the noise model directly from the measurements using the MAD
estimator, then derives all regularization weights analytically from
the Bayesian MAP framework:

    sw  ∝  1 / σ²_process   (smoothness = inverse innovation variance)
    rw  ∝  1 / σ²_noise     (regularization = inverse measurement noise)

No manual hyperparameter tuning required — the only inputs are the
measurements themselves and structural parameters (window size, anchor
spacing).

Usage:
    python -m experiments.exp35_auto_denoise \
        --sequences-dir /data/afnan/SemanticKitti/dataset/sequences --seq 00
    python -m experiments.exp35_auto_denoise \
        --sequences-dir /path/to/sequences
"""
...
def estimate_noise_model(measurements: np.ndarray) -> dict:
    ...
    """
    n_meas, dim = measurements.shape
    diffs = measurements[1:] - measurements[:-1]  # (n-1, 6)
...
def compute_auto_weights(noise_model: dict, *, base_sw: float = 1.0,
    ...
    base_sw and base_rw are overall scaling factors that control the
    absolute strength of the loss terms.  Their ratio is fixed by the
    noise model; only the overall scale needs tuning (and base_sw=1,
    base_rw=1 is a reasonable default).
    """
...
def main():
    parser = argparse.ArgumentParser(
        description="exp35: self-tuning SE(3) measurement denoiser")
    ...
    print("=" * 70)
    print("  exp35 -- Self-Tuning SE(3) Denoiser")
    print("=" * 70)
    _print_device_info()
    print()
