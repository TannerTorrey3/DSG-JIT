# experiments/exp41_refine_denoise.py
"""
Real-time differentiable SE(3) measurement denoiser with three-phase
optimization: translation → rotation → translation refinement.

Evolution of exp40 that addresses the two-phase coupling problem:
when rotation is optimized in phase 2, it changes the inner PGO
solution, shifting what the optimal translation should be.  Phase 1's
translation result is left stale.  This adds a short phase 3 that
lets translation readjust after rotation is settled.

Optimization schedule:
    Phase 1: Translation only (default 50 iters, fresh Adam)
    Phase 2: Rotation only    (default 20 iters, fresh Adam)
    Phase 3: Translation refinement (default 15 iters, fresh Adam)

All other machinery inherited from exp40: windowed noise decomposition,
one-time JIT compilation, dynamic weight passing.

Usage:
    python -m experiments.exp41_refine_denoise \\
        --sequences-dir /data/afnan/SemanticKitti/dataset/sequences --seq 00

    # Disable refinement phase (reduces to exp40 two-phase):
    python -m experiments.exp41_refine_denoise \\
        --sequences-dir /path/to/sequences --n-refine-iters 0
"""
...
# ---------------------------------------------------------------------------
# Bilevel denoiser — IFT + three-phase optimisation
# ---------------------------------------------------------------------------
...
def build_denoiser(
    ...
    *,
    ...
    """Build a JIT-compiled denoiser with IFT backward pass and three-phase Adam.

    Three-phase schedule: translation → rotation → translation refinement.
...
def denoise_sequence(
    ...
    n_total_iters = args.n_trans_iters + args.n_rot_iters + args.n_refine_iters
    ...
    print(f"  Outer loop: Three-phase ({args.n_trans_iters} trans + "
          f"{args.n_rot_iters} rot + {args.n_refine_iters} refine = "
          f"{n_total_iters} total)")
    ...
    for wi, (w_start, w_end) in enumerate(windows):
        ...
        # Run fused three-phase optimization (single compiled kernel).
...
def main():
    parser = argparse.ArgumentParser(
        description="exp41: real-time SE(3) denoiser with three-phase "
                    "optimization (trans → rot → trans refinement)")
    ...
    parser.add_argument("--n-refine-iters", type=int, default=15,
                        help="Phase 3 (translation refinement) Adam iterations")
    ...
    print("=" * 70)
    print("  exp41 -- SE(3) Denoiser with Three-Phase Optimization")
    print("=" * 70)
    _print_device_info()
    print()
    ...
    run_dir = os.path.join(args.output_dir, f"exp41_{timestamp}")
    ...
    config = {
        ...
        "aw_rot": args.aw_rot,
        "inner_anchor_sigma": args.inner_anchor_sigma,
        "outer_loop": f"three-phase ({args.n_trans_iters} trans + {args.n_rot_iters} rot + {args.n_refine_iters} refine)",
