...
class WorldModel:
    ...
    @telemetry_span(
        ...
        shape_fn=lambda self, **kw: {
            "graph.nodes_bucket": bucket_count(len(self.fg.variables)),
            "graph.edges_bucket": bucket_count(len(self.fg.factors)),
            "iterations_bucket": bucket_count(kw.get("iters", 300)),
        }
    )
    def optimize(
        self,
    ...
    @telemetry_span(
        component="world",
        op="build_residual",
        safe_args={"use_type_weights", "learn_odom", "learn_voxel_points"},
        shape_fn=lambda self, **kw: {
            "graph.nodes_bucket": bucket_count(len(self.fg.variables)),
            "graph.edges_bucket": bucket_count(len(self.fg.factors)),
        }
    ...
    @telemetry_span(component="world", op="build_residual_function_se3_odom_param_multi")
    ...
    def _build_state_index(self) -> Dict[NodeId, Tuple[int, int]]:
        # Note: _build_state_index is private (starts with _), so no telemetry
